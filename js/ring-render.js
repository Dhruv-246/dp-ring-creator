...
export function renderKamyaRing(canvas, { size, styleKey = 'classic', img, silhouette }) {
  ...
  const paint = () => {
    ...
    if (hasPhoto) drawInnerImage(ctx, img, { cx, cy, innerR });
    else drawSilhouette(ctx, { cx, cy, innerR, bg: (silhouette && silhouette.bg), fg: (silhouette && silhouette.fg), bust: (silhouette && silhouette.bust) || 0 });
    // Ring overlay on top (transparent inner disc lets the photo show through).
